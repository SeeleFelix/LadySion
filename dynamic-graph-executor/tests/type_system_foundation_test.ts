...
Deno.test("T1.1: 类型系统基础计算链验证", async () => {
  ...
  // 验证终端输出节点（经过过滤后）
  assertEquals(
    "starter.execution_id" in nodeOutputs,
    true,
    "应该包含starter节点的execution_id输出",
  );
  assertEquals(
    "timer.timestamp" in nodeOutputs,
    false,
    "timer.timestamp被完全消费，不应出现在终端输出中",
  );
  ...
  assertEquals(
    "timer.timestamp" in nodeOutputs,
    false,
    "timer.timestamp被其他节点消费，不应该在最终输出中",
  );
