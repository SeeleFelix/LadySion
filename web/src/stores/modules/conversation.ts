...
import type { Conversation, Message, CreateConversationData, SendMessageData } from '@/types'
...
export const useConversationStore = defineStore('conversation', () => {
  ...
  const currentMessages = computed(() => 
    currentConversation.value ? messages.value.filter(m => m.conversationId === currentConversation.value!.id) : []
  )
  ...
  const loadMessages = async (conversationId: string) => {
    ...
    error.value = null
    try {
      const conversationMessages = await conversationApi.getMessages(conversationId)
