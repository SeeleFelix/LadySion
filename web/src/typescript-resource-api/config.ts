...
import type { ResourceConfig, RealtimeConfig, ApiPaths } from './types'
 ...
 * 获取完整的基础资源配置
 */
export function getResourceConfig(userConfig?: Partial<ResourceConfig>): Required<ResourceConfig> {
 ...
 * 获取完整的实时资源配置
 */
export function getRealtimeConfig(userConfig?: Partial<RealtimeConfig>): Required<RealtimeConfig> {
  const envConfig = getEnvConfig()
 ...
 */
export function buildResourcePath(resourceName: string, config?: Partial<ResourceConfig>): string {
 ...
 */
export function buildRealtimePath(resourceName: string, config?: Partial<RealtimeConfig>): string {
 ...
 */
export function buildApiUrl(resourceName: string, config?: Partial<ResourceConfig>): string {
  const finalConfig = getResourceConfig(config)
 ...
 */
export function buildRealtimeUrl(resourceName: string, config?: Partial<RealtimeConfig>): string {
  const finalConfig = getRealtimeConfig(config)
