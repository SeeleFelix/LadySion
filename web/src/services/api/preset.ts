...
export { PresetType, PRESET_TYPE_LABELS }
...
class PresetService extends BaseApiService {
   ...
   * 获取指定类型的所有预设
   */
  async getAll(type: PresetType, params?: PresetQueryParams): Promise<Preset[]> {
  ...
  async getByName(type: PresetType, name: string): Promise<Preset | null> {
    try {
      return await this.get<Preset>(`/presets/${type}/name/${encodeURIComponent(name)}`)
    } catch (error: any) {
      if (error.status === 404) {
        return null
      }
   ...
   * 创建新预设
   */
  async create(type: PresetType, preset: Omit<Preset, 'id' | 'createdAt' | 'updatedAt'>): Promise<Preset> {
   ...
   * 更新预设
   */
  async update(type: PresetType, id: string, preset: Partial<Preset>): Promise<Preset> {
   ...
   * 验证预设数据
   */
  async validate(type: PresetType, preset: Partial<Preset>): Promise<PresetValidationResult> {
    return this.post<PresetValidationResult>(`/presets/${type}/validate`, preset)
  }
   ...
   * 更新预设配置
   */
  async updateConfiguration(config: Partial<PresetConfiguration>): Promise<PresetConfiguration> {
  ...
  async selectInstructPreset(id: string): Promise<PresetOperationResult> {
    return this.post<PresetOperationResult>(`/presets/${PresetType.INSTRUCT}/select/${id}`)
  }
  ...
  async selectContextPreset(id: string): Promise<PresetOperationResult> {
    return this.post<PresetOperationResult>(`/presets/${PresetType.CONTEXT}/select/${id}`)
  }
  ...
  async selectSystemPromptPreset(id: string): Promise<PresetOperationResult> {
    return this.post<PresetOperationResult>(`/presets/${PresetType.SYSTEM_PROMPT}/select/${id}`)
  }
  ...
  async exportMasterPreset(name: string): Promise<MasterPreset> {
    return this.get<MasterPreset>(`/presets/master/export/${encodeURIComponent(name)}`)
  }

  /**
   * 创建主预设
   */
  async createMasterPreset(masterPreset: Omit<MasterPreset, 'exportedAt' | 'importedAt'>): Promise<MasterPreset> {
   ...
   * 批量导入预设
   */
  async importBatch(type: PresetType, presets: Preset[]): Promise<ApiResponse<Preset[]>> {
    return this.post<ApiResponse<Preset[]>>(`/presets/${type}/batch/import`, { presets })
  }
  ...
  async search(query: string, types?: PresetType[]): Promise<Preset[]> {
    ...
    return this.get<Preset[]>(`/presets/search${this.buildQueryString(params)}`)
  }
