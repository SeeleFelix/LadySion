...
Deno.test({
  ...
  async fn() {
    ...
    try {
       ...
       assert(stats.total >= initialStats.total + 4, `期望总任务数至少为 ${initialStats.total + 4}，实际为 ${stats.total}`);
       assert(stats.completed >= initialStats.completed + 2, `期望完成任务数至少为 ${initialStats.completed + 2}，实际为 ${stats.completed}`);
