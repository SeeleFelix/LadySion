...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      const initialCompleted = completedTasks.filter((t) =>
      ...
      const initialIncomplete = incompleteTasks.filter((t) =>
        tasks.some((task) => task.id === t.id)
      ).length;
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      assert(
        allProjects.length === initialCount + 1,
        `项目总数应该增加1个，期望 ${initialCount + 1}，实际 ${allProjects.length}`,
      );
