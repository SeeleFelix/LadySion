...
use anima_weave_core::{
    error_handling::BusinessError, types::PortRef, AnimaWeaveError, NodeDataInputs,
    NodeDataOutputs, SemanticLabel,
};
...
static ADD_NODE_INFO: once_cell::sync::Lazy<NodeInfo> = once_cell::sync::Lazy::new(|| NodeInfo {
    ...
    input_ports: vec![
        PortDef::required_data("a", "Number"),
        PortDef::required_data("b", "Number"),
    ],
    output_ports: vec![PortDef::output_data("result", "Number")],
});
...
impl Node for AddNode {
    ...
    fn execute(&self, inputs: &NodeDataInputs) -> Result<NodeDataOutputs, AnimaWeaveError> {
        ...
        // 获取输入参数
        let a_port = PortRef::new("add", "a");
        let b_port = PortRef::new("add", "b");

        let a = inputs.get(&a_port).ok_or_else(|| {
            AnimaWeaveError::Business(BusinessError::InputValidation {
                port: "a".to_string(),
                reason: "Missing required input 'a'".to_string(),
            })
        })?;

        let b = inputs.get(&b_port).ok_or_else(|| {
            AnimaWeaveError::Business(BusinessError::InputValidation {
                port: "b".to_string(),
                reason: "Missing required input 'b'".to_string(),
            })
        })?;

        // 尝试转换为NumberLabel
        let a_number = a.as_any().downcast_ref::<NumberLabel>().ok_or_else(|| {
            AnimaWeaveError::Business(BusinessError::TypeConversion {
                from_type: a.get_semantic_label_type().to_string(),
                to_type: "NumberLabel".to_string(),
                reason: "Input 'a' must be a number".to_string(),
            })
        })?;

        let b_number = b.as_any().downcast_ref::<NumberLabel>().ok_or_else(|| {
            AnimaWeaveError::Business(BusinessError::TypeConversion {
                from_type: b.get_semantic_label_type().to_string(),
                to_type: "NumberLabel".to_string(),
                reason: "Input 'b' must be a number".to_string(),
            })
        })?;
        ...
        outputs.insert(PortRef::new("add", "result"), Arc::new(result));
...
mod tests {
    ...
    use anima_weave_core::{types::PortRef, SemanticLabel};
    ...
    fn test_add_node_addition() {
        ...
        inputs.insert(
            PortRef::new("add", "a"),
            Arc::new(NumberLabel { value: 5.0 }),
        );
        inputs.insert(
            PortRef::new("add", "b"),
            Arc::new(NumberLabel { value: 3.0 }),
        );
        ...
        let result_port = PortRef::new("add", "result");
    ...
    fn test_add_node_missing_input() {
        ...
        match result.unwrap_err() {
            AnimaWeaveError::Business(BusinessError::InputValidation { port, .. }) => {
                assert_eq!(port, "b");
            }
