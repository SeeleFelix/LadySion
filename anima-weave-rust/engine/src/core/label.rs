...
macro_rules! semantic_label {
    ...
    ($(#[$attr:meta])* $name:ident($($field:ident: $field_type:ty),* $(,)?) {
        ...
        impl $crate::core::label::SemanticLabel for $name {
            fn clone_box(&self) -> Box<dyn $crate::core::label::SemanticLabel> {
                Box::new(self.clone())
            }
            ...
            fn conversion_map(&self) -> std::collections::HashMap<&'static str, $crate::core::label::ConversionFn> {
                #[allow(unused_mut)]
                ...
                $(
                    ...
                    map.insert(target_type_name, Box::new(|any: &dyn std::any::Any| -> Result<Box<dyn $crate::core::label::SemanticLabel>, $crate::core::label::TransformError> {
                        if let Some($self_param) = any.downcast_ref::<$name>() {
                            // 确保转换结果类型正确
                            let result: $target_type = $conversion;
                            Ok(Box::new(result) as Box<dyn $crate::core::label::SemanticLabel>)
                        } else {
                            Err($crate::core::label::TransformError::ConversionFailed {
                                reason: format!("Failed to downcast to {}", stringify!($name)),
                            })
                        }
                    }) as $crate::core::label::ConversionFn);
                )*
