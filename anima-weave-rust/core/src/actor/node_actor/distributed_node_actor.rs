...
impl DistributedNodeActor {
    ...
    async fn start_execution(&mut self, ctx: &mut Context<Self, ()>) -> Result<(), String> {
        ...
        match self.execute_node().await {
            Ok(outputs) => {
                self.handle_execution_success(execution_id, outputs, ctx).await;
            }
            Err(error) => {
                self.handle_execution_failure(execution_id, error, ctx).await;
            }
        }
    ...
    /// 执行节点的具体逻辑
    async fn execute_node(&mut self) -> Result<HashMap<PortRef, Vec<Arc<dyn SemanticLabel>>>, String> {
    ...
    async fn handle_execution_success(
        ...
        for (port, data_list) in outputs {
            self.send_data_to_downstream(port, data_list, execution_id.clone(), ctx).await;
        }
...
impl Message<DataMessage> for DistributedNodeActor {
    ...
    async fn handle(&mut self, message: DataMessage, _ctx: &mut Context<Self, Self::Reply>) -> Self::Reply {
        let start_time = SystemTime::now();

        // 存储数据
        self.pending_data_inputs.insert(message.to_port, message.data);

        // 记录处理时间
        if let Ok(processing_time) = SystemTime::now().duration_since(start_time) {
            self.message_processing_times.push(processing_time);
            let _ = self.status_collector.tell(NodeStatusEvent::MessageProcessed {
                node_name: self.node_name.clone(),
...
impl Message<ControlMessage> for DistributedNodeActor {
    ...
    async fn handle(&mut self, message: ControlMessage, _ctx: &mut Context<Self, Self::Reply>) -> Self::Reply {
        let start_time = SystemTime::now();

        // 存储控制信号
        self.pending_control_inputs.insert(message.to_port, message.signal);
...
impl Message<GetNodeStateQuery> for DistributedNodeActor {
    ...
    async fn handle(&mut self, _query: GetNodeStateQuery, _ctx: &mut Context<Self, Self::Reply>) -> Self::Reply {
        self.get_state()
    }
}
...
impl Message<GetNodeInfoQuery> for DistributedNodeActor {
    ...
    async fn handle(&mut self, _query: GetNodeInfoQuery, _ctx: &mut Context<Self, Self::Reply>) -> Self::Reply {
        NodeInfo {
