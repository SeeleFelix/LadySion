...
impl StatusCollector {
    ...
    pub fn get_execution_record(&self, execution_id: &ExecutionId) -> Option<&DetailedExecutionRecord> {
    ...
    fn handle_status_event(&mut self, event: NodeStatusEvent) {
        ...
        match event {
            NodeStatusEvent::ExecutionQueued { node_name, execution_id, is_sequential } => {
                self.handle_execution_queued(node_name, execution_id, is_sequential);
            }
            NodeStatusEvent::ExecutionStarted { node_name, execution_id, input_count } => {
                self.handle_execution_started(node_name, execution_id, input_count);
            }
            NodeStatusEvent::ExecutionCompleted { node_name, execution_id, output_count, duration } => {
                self.handle_execution_completed(node_name, execution_id, output_count, duration);
            }
            NodeStatusEvent::ExecutionFailed { node_name, execution_id, error, duration } => {
            ...
            NodeStatusEvent::MessageProcessed { node_name, message_type, processing_time } => {
                self.handle_message_processed(node_name, message_type, processing_time);
            }
        }
    }
    
    fn handle_execution_queued(&mut self, node_name: NodeName, execution_id: ExecutionId, is_sequential: bool) {
    ...
    fn handle_execution_completed(&mut self, node_name: NodeName, execution_id: ExecutionId, output_count: usize, duration: Duration) {
    ...
    fn handle_execution_failed(&mut self, node_name: NodeName, execution_id: ExecutionId, error: String, duration: Duration) {
        if let Some(record) = self.active_executions.remove(&execution_id) {
            self.complete_execution(record, false, 0, duration, Some(error));
        }
    }
    
    fn handle_message_processed(&mut self, node_name: NodeName, message_type: MessageType, processing_time: Duration) {
        match message_type {
        ...
        if total_messages > 0 {
            let current_avg_nanos = self.message_stats.average_message_processing_time.as_nanos() as u64;
            ...
            self.message_stats.average_message_processing_time = Duration::from_nanos(new_avg_nanos);
        }
    }
    
    fn complete_execution(&mut self, mut record: DetailedExecutionRecord, success: bool, output_count: usize, duration: Duration, error: Option<String>) {
        // 更新记录
        ...
        if success {
            ...
            self.current_error = Some(format!("Node '{}': {}", 
                record.node_name, 
                error_msg
            ));
        }
        ...
        // 更新完整记录
        self.all_execution_records.insert(record.execution_id.clone(), record);
    ...
    fn update_node_statistics(&mut self, record: &DetailedExecutionRecord) {
        let stats = self.node_statistics.entry(record.node_name.clone()).or_default();
        
        stats.total_executions += 1;
        if matches!(record.status, DetailedExecutionStatus::Completed { success: true }) {
        ...
        if let Some(duration) = record.execution_duration {
            ...
            stats.average_execution_time = stats.total_processing_time / stats.total_executions as u32;
    ...
    fn get_top_performing_nodes(&self, limit: usize) -> Vec<(NodeName, NodeStats)> {
        let mut nodes: Vec<_> = self.node_statistics.iter()
    ...
    fn identify_bottlenecks(&self) -> Vec<(NodeName, String)> {
        ...
        for (node_name, stats) in &self.node_statistics {
            let success_rate = stats.successful_executions as f64 / stats.total_executions.max(1) as f64;
            
            if success_rate < 0.8 {
                bottlenecks.push((node_name.clone(), format!("Low success rate: {:.2}%", success_rate * 100.0)));
            }
            
            if stats.average_execution_time > Duration::from_secs(5) {
                bottlenecks.push((node_name.clone(), format!("Slow execution: {:?}", stats.average_execution_time)));
            }
        }
...
impl Actor for StatusCollector {
    ...
    async fn on_start(collector: Self::Args, _actor_ref: ActorRef<Self>) -> Result<Self, Self::Error> {
...
impl Message<NodeStatusEvent> for StatusCollector {
    ...
    async fn handle(&mut self, event: NodeStatusEvent, _ctx: &mut Context<Self, Self::Reply>) -> Self::Reply {
...
impl Message<GetStatusQuery> for StatusCollector {
    ...
    async fn handle(&mut self, _query: GetStatusQuery, _ctx: &mut Context<Self, Self::Reply>) -> Self::Reply {
        self.get_status()
    }
}
...
impl Message<GetNodeStatsQuery> for StatusCollector {
    ...
    async fn handle(&mut self, query: GetNodeStatsQuery, _ctx: &mut Context<Self, Self::Reply>) -> Self::Reply {
...
impl Message<GetPerformanceReportQuery> for StatusCollector {
    ...
    async fn handle(&mut self, _query: GetPerformanceReportQuery, _ctx: &mut Context<Self, Self::Reply>) -> Self::Reply {
...
impl Message<ResetStatsCommand> for StatusCollector {
    ...
    async fn handle(&mut self, _command: ResetStatsCommand, _ctx: &mut Context<Self, Self::Reply>) -> Self::Reply {
        self.reset_stats();
    }
}
