use super::types::{NodeStatus, EventMeta};
use crate::types::{NodeName, ExecutionId, NodeInputs};
...
impl NodeExecutionEvent {
    ...
    /// 创建开始执行事件
    pub fn started(node_name: impl Into<NodeName>, node_execute_id: impl Into<ExecutionId>) -> Self {
        Self::new(node_name, node_execute_id, NodeStatus::Running)
    }
    
    /// 创建完成事件
    pub fn completed(node_name: impl Into<NodeName>, node_execute_id: impl Into<ExecutionId>) -> Self {
    ...
    /// 创建等待事件
    pub fn pending(node_name: impl Into<NodeName>, node_execute_id: impl Into<ExecutionId>) -> Self {
...
mod tests {
    ...
    fn test_node_execute_event_with_inputs() {
        ...
        inputs.insert("input1".to_string(), Box::new(SignalLabel::active()) as Box<dyn crate::SemanticLabel>);
        inputs.insert("input2".to_string(), Box::new(SignalLabel::inactive()) as Box<dyn crate::SemanticLabel>);
    ...
    fn test_node_execute_event_get_input() {
        let mut inputs = HashMap::new();
        inputs.insert("signal_port".to_string(), Box::new(SignalLabel::active()) as Box<dyn crate::SemanticLabel>);
