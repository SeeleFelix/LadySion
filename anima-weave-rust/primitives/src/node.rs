...
use crate::{SemanticLabel, PortCollection, AnimaError};
...
pub trait NodeExecution: Send {
    ...
    fn result(&self) -> Option<Result<HashMap<String, Box<dyn SemanticLabel>>, Box<dyn AnimaError>>>;
    ...
    fn into_future(self) -> Pin<Box<dyn Future<Output = Result<HashMap<String, Box<dyn SemanticLabel>>, Box<dyn AnimaError>>> + Send>>
    where
        Self: Sized + 'static;
}
...
pub trait Node: Send + Sync {
    ...
    fn execute_boxed(&self, inputs: HashMap<String, Box<dyn SemanticLabel>>) -> Box<dyn NodeExecution>;
