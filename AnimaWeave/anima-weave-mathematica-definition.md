...
**标准读法**：
> "斐函数：对于所有属于D_in的端口p和类型tau的笛卡尔积，映射到所有属于D_out的端口p和类型tau的笛卡尔积"

**简化读法**：
> "斐函数：输入端口的类型积映射到输出端口的类型积"
...
**含义解释**：φ是一个函数，接受类型为`A×B`的输入（即一个对`(a,b)`），返回类型为`C×D`的输出（即一个对`(c,d)`）。
