...
public class UUIDLabel extends SemanticLabel<UUID> {
     ...
     * 静态转换函数 - 复用同一个实例，提高性能
     */
    private static final Function<Object, UUID> CONVERTER = value -> {
        ...
        throw new IllegalArgumentException("无法将类型 " + value.getClass().getSimpleName() + " 转换为UUID");
    };
    ...
    @Override
    public SemanticLabel<UUID> withValue(UUID newValue) {
        return new UUIDLabel(newValue);
    }
    
    @Override
    public Function<Object, UUID> getConverter() {
