 ...
 * 
 * 职责：
 * 1. 根据vessel提供的NodeDefinition创建NodeInstance
 * 2. 将NodeInstance动态注册到Spring容器
 * 3. 确保每个节点类型都有对应的监听器
 */
@Slf4j
@Component
...
public class NodeInstanceFactory {
    ...
    public void createNodeInstancesForVessel(AnimaVessel vessel) {
        ...
        for (NodeDefinition nodeDef : vessel.getSupportedNodes()) {
            try {
                ...
                log.error("Failed to create NodeInstance for node type: {} in vessel: {}", 
                         nodeDef.nodeType(), vesselName, e);
            }
        }
    }
    ...
    public static class GenericNodeInstance extends NodeInstance {
        ...
        protected Map<String, Object> executeNode(Map<String, Object> inputs) {
            ...
            var result = inputs.values().stream()
