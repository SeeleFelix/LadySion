 ...
 * 节点基类 - 使用注解标识端口和元数据
 *
 * <p>子类通过@NodeMeta注解定义元数据 子类通过@InputPort和@OutputPort注解标识端口字段 字段类型必须是Port<? extends
 * SemanticLabel<?>>
 *
 * <p>新的设计： - Port字段定义端口规格（名称、类型、是否必需等） - 运行时框架将SemanticLabel实例注入到对应的输入字段 -
...
public abstract class Node {
  ...
  /** 子类实现的具体节点逻辑 输入数据通过getInput()方法获取 输出数据通过setOutput()方法设置 */
  ...
  private NodeOutputSaveEvent createSuccessEvent(
      String nodeId,
      String executionId,
      Map<String, SemanticLabel<?>> outputs,
      String executionContextId) {
