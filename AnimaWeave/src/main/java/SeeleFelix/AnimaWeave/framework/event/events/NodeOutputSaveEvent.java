 ...
 * 节点输出保存事件 - 统一的节点完成事件
 * 
 * 所有节点执行完成后都发送这个相同的事件类型
 * GraphCoordinator监听此事件来：
 * 1. 更新数据总线
 * 2. 决定下一个执行的节点
 */
@Getter
@ToString(callSuper = true, of = {"nodeName", "nodeExecutionId", "outputsSize"})
@EqualsAndHashCode(callSuper = false, of = {"nodeName", "nodeExecutionId", "graphExecutionId"})
public final class NodeOutputSaveEvent extends AnimaWeaveEvent {
    ...
    public NodeOutputSaveEvent(Object source, String sourceIdentifier,
                              String nodeName, String nodeExecutionId,
                              Map<String, Object> outputs, String graphExecutionId) {
     ...
     * 便捷的静态工厂方法
     */
    public static NodeOutputSaveEvent of(Object source, String sourceIdentifier,
                                        String nodeName, String nodeExecutionId,
                                        Map<String, Object> outputs, String graphExecutionId) {
        return new NodeOutputSaveEvent(source, sourceIdentifier, nodeName, 
                                     nodeExecutionId, outputs, graphExecutionId);
    }
    ...
    public String getExecutionSummary() {
        return "Node '%s' (exec: %s) completed with %d outputs: %s".formatted(
            nodeName, nodeExecutionId, outputs.size(), outputs.keySet()
        );
    }
} 