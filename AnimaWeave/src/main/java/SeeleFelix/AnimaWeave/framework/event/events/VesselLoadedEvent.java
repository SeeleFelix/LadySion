...
/**
 * Vessel加载完成事件
 * 当一个vessel插件成功加载并初始化完成时发出
 * 使用lombok + 静态工厂方法的实用现代化方案
 */
@Getter
@ToString(callSuper = true, of = {"vesselName", "successful", "errorMessage"})
...
public class VesselLoadedEvent extends AnimaWeaveEvent {
    ...
    // 私有主构造函数
    private VesselLoadedEvent(Object source, String sourceIdentifier, String vesselName, 
                            VesselMetadata metadata, boolean successful, String errorMessage) {
    ...
    public static VesselLoadedEvent success(Object source, String sourceIdentifier, 
                                          String vesselName, VesselMetadata metadata) {
        return new VesselLoadedEvent(source, sourceIdentifier, vesselName, metadata, true, null);
    }
    
    // Builder风格的静态工厂方法 - 加载失败
    public static VesselLoadedEvent failure(Object source, String sourceIdentifier, 
                                          String vesselName, String errorMessage) {
