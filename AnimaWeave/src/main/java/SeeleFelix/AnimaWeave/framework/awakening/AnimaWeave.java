 ...
 * AnimaWeave - 图执行的核心引擎
 * 
 * 这是AnimaWeave系统的主入口，负责解析和执行图文件
 * 框架自动加载内置的.anima容器文件，用户只需提供.weave图文件
 */
@Slf4j
@Component
...
public class AnimaWeave {
    ...
    public CompletableFuture<AwakeningResult> awakening(File[] weaveFiles, String mainGraphName) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                log.info("🌅 AnimaWeave awakening begins with {} weave files, main graph: {}", 
                    weaveFiles.length, mainGraphName != null ? mainGraphName : "auto-detect");
     ...
     * 框架自动处理.anima容器文件的加载
     */
    private GraphDefinition parseWeaveFiles(File[] weaveFiles, String mainGraphName) throws Exception {
