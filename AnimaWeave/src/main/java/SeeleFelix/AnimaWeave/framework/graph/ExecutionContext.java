...
public class ExecutionContext {
    ...
    private final List<NodeExecutionRecord> executionHistory = Collections.synchronizedList(new ArrayList<>());
     ...
     * 开始节点执行 - 记录开始时间和执行信息
     */
    public NodeExecutionRecord startNodeExecution(String nodeName, String nodeType, Map<String, Object> inputs) {
        ...
        log.debug("🚀 节点开始执行: {} (#{}, 第{}次) - 执行ID: {}", 
                 nodeName, globalSequence, nodeCount, executionId);
     ...
     * 完成节点执行 - 记录结束时间和输出结果
     */
    public NodeExecutionRecord completeNodeExecution(String executionId, Map<String, Object> outputs) {
        ...
        var completedRecord = runningRecord.withCompletion(endTime, duration, NodeExecutionStatus.SUCCESS, outputs, null);
        ...
        log.debug("✅ 节点执行完成: {} (#{}, 耗时{}ms) - 执行ID: {}", 
                 runningRecord.nodeName(), runningRecord.globalSequence(), 
                 duration.toMillis(), executionId);
    ...
    public NodeExecutionRecord failNodeExecution(String executionId, String errorMessage) {
        ...
        var failedRecord = runningRecord.withCompletion(endTime, duration, NodeExecutionStatus.FAILED, null, errorMessage);
    ...
    public ExecutionSummary getExecutionSummary() {
        ...
        var successCount = (int) executionHistory.stream()
            .filter(record -> record.status == NodeExecutionStatus.SUCCESS)
            .count();
            
        var failedCount = (int) executionHistory.stream()
    ...
    public record ExecutionSummary(
        ...
        public String getStatusSummary() {
            return String.format("总计: %d, 成功: %d, 失败: %d, 运行中: %d", 
                               totalNodes, successNodes, failedNodes, runningNodes);
        }
    }
    ...
    public static class NodeExecutionRecord {
        ...
        public NodeExecutionRecord(String nodeExecutionId, long globalSequence, int nodeSequence,
                                 String nodeName, String nodeType, Map<String, Object> inputs,
                                 Instant startTime, Instant endTime, Duration duration,
                                 NodeExecutionStatus status, Map<String, Object> outputs, String errorMessage) {
        ...
        public NodeExecutionRecord withCompletion(Instant endTime, Duration duration, 
                                                NodeExecutionStatus status, Map<String, Object> outputs, String errorMessage) {
            return new NodeExecutionRecord(
                this.nodeExecutionId, this.globalSequence, this.nodeSequence,
                this.nodeName, this.nodeType, this.inputs,
                this.startTime, endTime, duration, status, outputs, errorMessage
            );
        }
        
        // Getters
        public String nodeExecutionId() { return nodeExecutionId; }
        public long globalSequence() { return globalSequence; }
        public int nodeSequence() { return nodeSequence; }
        ...
        public String toString() {
            return String.format("NodeExecution[#%d %s %s %s]", 
