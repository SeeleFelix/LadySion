...
/** DSL图构建器 使用ANTLR Visitor模式来构建图定义，基于Pest语法转换的ANTLR4语法 */
@Slf4j
public class DSLGraphBuilder extends AnimaWeaveDSLBaseVisitor<GraphDefinition> {
  ...
  private void parsePortSections(
      AnimaWeaveDSLParser.PortSectionsContext ctx,
  ...
  private void parseInSection(
      AnimaWeaveDSLParser.InSectionContext ctx, Map<String, String> inputPorts) {
  ...
  private void parseOutSection(
      AnimaWeaveDSLParser.OutSectionContext ctx, Map<String, String> outputPorts) {
    if (ctx.portList() != null) {
      for (var portDef : ctx.portList().portDefinition()) {
        String portName = portDef.IDENTIFIER().getText();
        String portType = portDef.qualifiedName().getText();
  ...
  @Override
  public GraphDefinition visitNodesInstanceSection(
      AnimaWeaveDSLParser.NodesInstanceSectionContext ctx) {
  ...
  @Override
  public GraphDefinition visitDataConnectionSection(
      AnimaWeaveDSLParser.DataConnectionSectionContext ctx) {
    // 访问所有数据连接
  ...
  @Override
  public GraphDefinition visitControlConnectionSection(
      AnimaWeaveDSLParser.ControlConnectionSectionContext ctx) {
    // 访问所有控制连接
