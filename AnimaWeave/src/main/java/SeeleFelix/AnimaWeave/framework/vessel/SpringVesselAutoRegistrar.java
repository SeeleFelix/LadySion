 ...
 * Spring容器Vessel自动注册器
 * 
 * 专门负责发现和注册Spring容器中的@Component vessel
 * 职责单一：只处理Spring bean形式的vessel
 */
@Slf4j
@Component
...
public class SpringVesselAutoRegistrar {
    ...
    public void registerSpringVessels() {
        ...
        for (Map.Entry<String, AnimaVessel> entry : vesselBeans.entrySet()) {
            ...
            try {
                ...
                log.info("✅ 自动注册@Component vessel: {} v{} (bean: {})", 
                        vesselId, vessel.getMetadata().version(), beanName);
    ...
    public VesselRegistrationStats getRegistrationStats() {
        ...
        return new VesselRegistrationStats(
            vesselNames.size(),
            vesselNames
        );
    }
     ...
     * Vessel注册统计信息
     */
    public record VesselRegistrationStats(
        int totalCount,
        java.util.List<String> vesselNames
    ) {
        @Override
