...
public class VesselManager {
  ...
  /** Spring启动时自动加载JAR vessel插件 职责单一：只负责JAR文件的vessel加载 */
  @PostConstruct
        ...
        .whenComplete(
            (result, throwable) -> {
              if (throwable != null) {
                ...
                log.info(
                    "📊 Current JAR vessels in registry: {}",
                    vesselRegistry.getVesselNames().size());
              }
            });
  }
  ...
  public CompletableFuture<Void> loadAllVessels() {
    ...
    return CompletableFuture.allOf(loadTasks)
        .thenRun(
            () ->
                log.info("Loaded {} vessels successfully", vesselRegistry.getAllVessels().size()));
  }
    ...
    try {
      ...
      for (var vessel : serviceLoader) {
        try {
          ...
          log.info(
              "Successfully loaded JAR vessel: {} v{}",
  ...
  public CompletableFuture<Void> shutdown() {
    ...
    var shutdownTasks =
        vesselRegistry.getVesselNames().stream()
