...
/**
 * Vessel注册表实现 线程安全的vessel插件注册表 使用lombok和Java 21现代化实现
 *
 * <p>增强版本：统一处理vessel注册和事件发布
 */
@Slf4j
@Component
...
public class VesselRegistryImpl implements VesselRegistry {
  ...
  /** 发布vessel加载事件 统一事件发布逻辑，避免各处重复代码 */
  private void publishVesselLoadedEvent(String vesselName, AnimaVessel vessel) {
    try {
      var event =
          VesselLoadedEvent.success(this, "VesselRegistry", vesselName, vessel.getMetadata());
