...
本文档定义了NodeFlow双流系统的领域特定语言(DSL)设计，建立了DSL语法与数学形式化定义的完整对应关系，并明确了三层架构中各层的职责分工。
...
| 10 | NodeFlow图三元组 | `-- graph` 结构 | 图结构验证 | 图执行调度 |
...
| 定义 | 概念 | DSL语法 | DSL检查 | Rust引擎 |
|------|------|---------|---------|----------|
| 16 | 双流NodeFlow系统 | ✅ 整个DSL体现 | ✅ 完整性检查 | ✅ 系统实例化 |
...
| 定义 | 概念 | DSL语法 | DSL检查 | Rust引擎 |
|------|------|---------|---------|----------|
| 25 | 子图关系 | `.grph.nodeflow` 文件 | 子图导入解析 | 子图实例化 |
...
### ✅ 数学完备性
DSL完整对应了NodeFlow数学形式化定义的32个概念，确保了理论与实现的一致性。
...
- [NodeFlow数学形式化定义](./nodeflow-mathematica-definition.md)
