...
import { SendMessageUseCase } from "../usecases/SendMessageUseCase.ts";
...
import { NotFoundError } from "@/shared/errors/DomainErrors.ts";
  ...
  async createChat(command: CreateChatCommand): Promise<Result<ChatDto>> {
    try {
      ...
      const title = command.title ||
        (character ? `与${character.name}的对话` : "新对话");
      ...
      if (character?.firstMessage) {
        ...
        const message = Message.fromData(welcomeMessage);
