 ...
 * 创建默认指令预设
 */
async function createDefaultInstructPresets(
  presetUseCases: PresetUseCases,
): Promise<void> {
  ...
  for (const preset of defaultInstructs) {
    const existing = await presetUseCases.getPresetById(
      PresetType.INSTRUCT,
      preset.id!,
    );
    if (!existing) {
 ...
 * 创建默认上下文预设
 */
async function createDefaultContextPresets(
  presetUseCases: PresetUseCases,
): Promise<void> {
 ...
 * 创建默认系统提示词预设
 */
async function createDefaultSystemPromptPresets(
  presetUseCases: PresetUseCases,
): Promise<void> {
  const defaultSystemPrompts: Partial<SystemPromptPreset>[] = [
    {
      ...
      content:
        "你是一个有用、无害、诚实的AI助手。请尽力回答用户的问题，提供准确和有用的信息。如果你不确定某个答案，请诚实地说明。",
      category: "general",
      tags: ["通用", "助手"],
      enabled: true,
      priority: 1,
    ...
    },
    {
      ...
      content:
        "你是一个富有创意和想象力的AI助手。你擅长创意写作、故事创作、诗歌和其他创意任务。请发挥你的创造力，为用户提供独特和有趣的内容。",
      category: "creative",
      tags: ["创意", "写作"],
      enabled: false,
      priority: 2,
    ...
    },
    {
      ...
      content:
        "你是一个技术专家AI助手。你在编程、软件开发、系统管理和技术问题解决方面有深厚的知识。请提供准确、详细的技术解答和建议。",
      category: "technical",
      tags: ["技术", "编程"],
      enabled: false,
      priority: 3,
