...
import { PresetType, Preset, PresetConfiguration } from '../../domain/entities/Preset';
...
export class SQLitePresetRepository implements PresetRepository {
   ...
   * 获取配置
   */
  async getConfiguration(key: string): Promise<PresetConfiguration | null> {
    if (!this.db) throw new Error('数据库未初始化');
   ...
   * 保存配置
   */
  async saveConfiguration(key: string, config: PresetConfiguration): Promise<void> {
    if (!this.db) throw new Error('数据库未初始化');
  ...
  private deserializePreset(row: any): Preset {
    ...
    preset.createdAt = new Date(row.created_at);
    preset.updatedAt = new Date(row.updated_at);
    return preset;
  }
