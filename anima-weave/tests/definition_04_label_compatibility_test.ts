...
import { awakening, ExecutionStatus, isStaticError, type ExecutionTrace } from "../src/mod.ts";
...
describe("定义4：语义标签兼容性关系 (Γ)", () => {
  ...
  describe("T4.1.2: 基础兼容性验证 - UUID与String的兼容", () => {
    it("应该验证(UUID, String) ∈ Γ的基础兼容性", async () => {
      ...
      const result = await awakening("./sanctums/definition_04", "T4_1_2_uuid_string_compatibility");
  ...
  describe("T4.1.4: 传递性验证 - Prompts->prompt->string链式转换", () => {
    it("应该验证Prompts->prompt->string的链式兼容性转换", async () => {
      ...
      const result = await awakening("./sanctums/definition_04", "T4_1_4_prompts_chain_test");
            ...
            // TDD: 明确期望链式转换成功
      assertEquals(result.status, ExecutionStatus.Success, "Prompts->prompt->string链式转换必须成功");
  ...
  describe("T4.1.5: 兼容性封闭验证 - 转换函数存在性", () => {
    it("应该验证每个兼容关系都有对应的转换实现", async () => {
      ...
      assertEquals(trace.executionOrder.length, 2, "应该执行两个节点");
      assertEquals(trace.executionOrder[0].nodeId, "starter", "第一个应该是starter节点");
      assertEquals(trace.executionOrder[1].nodeId, "uuid_to_string", "第二个应该是uuid_to_string节点");
      
      // 验证转换实现的正确性
      const closureOutputs = result.getOutputs();
      assertExists(closureOutputs["uuid_to_string.formatted"], "应该有格式化输出");
      assertEquals(closureOutputs["uuid_to_string.formatted"].semantic_label, "basic.String", "输出应该是String类型");
  ...
  describe("T4.1.6: 兼容性关系的完整性", () => {
    it("应该验证系统中所有兼容关系的一致性", async () => {
      ...
      for (const testKey of compatibilityTests) {
        ...
        if (typeof testResult === 'object' && testResult !== null && 'semantic_label' in testResult) {
      ...
      assertEquals(trace.executionOrder[1].nodeId, "compatibility_test", "第二个应该是compatibility_test");
      assertEquals(trace.executionOrder[2].nodeId, "completeness_marker", "第三个应该是completeness_marker");
      ...
      const testOutputs = Object.keys(completenessOutputs).filter(key => key.includes("compatibility_test"));
