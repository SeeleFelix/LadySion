...
describe("定义1：语义标签集合 (ℒ)", () => {
  ...
  describe("T1.1.2: 组合语义标签的构造", () => {
    it("应该正确构造和嵌套复杂的组合类型", async () => {
      ...
      assertEquals(
        promptValue.content.semantic_label,
        "basic.String",
        "content字段应该是String语义标签",
      );
  ...
  describe("T1.1.7: 序列化一致性", () => {
    it("应该在多次执行中保持语义标签格式的一致性", async () => {
      ...
      // Given: 执行同一图多次
      const result1 = await awakening(
        "./sanctums/definition_01",
        "T1_1_7_serialization_consistency",
      );
      const result2 = await awakening(
        "./sanctums/definition_01",
        "T1_1_7_serialization_consistency",
      );
      ...
      for (let i = 0; i < keys1.length; i++) {
        ...
        if (
          typeof output1 === "object" && typeof output2 === "object" &&
