...
import { AnimaVessel, Node, Port, SemanticLabel } from "../src/framework/core.ts";
...
export class ConstantNode extends Node {
  ...
  readonly inputs: Port[] = [
    new Port("value", NumberLabel), // 参数输入
  ];
  ...
  execute(inputPorts: Port[]): Port[] {
    // 从参数或输入中获取值
    const valuePort = inputPorts.find(p => p.name === "value");
    if (!valuePort || !valuePort.getValue()) {
      throw new Error("Constant节点需要value参数");
    }

    const value = valuePort.getValue()!.value;
